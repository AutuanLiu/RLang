...
getInfo <- function(value, minv, maxv) {
  ...
  abnormal.index <-
    which(!(value >= minv & value <= maxv & !is.na(value))) # 异常值索引
  # 三者之间存在交叉，所以取他们的交集
  # error.index <- Reduce(intersect, list(zero.index, miss.index, abnormal.index))
  # 由于很可能存在空集，造成错误，所以采用以下方法解决
  error.index <- c()
  error.index <-
    unique(append(error.index, c(zero.index, miss.index, abnormal.index)))
...
for (i in 1:len) {
  xx <- cbind(
    ...
    result[i]$info$HR$zero.rate,
    result[i]$info$ABPS$zero.rate,
    result[i]$info$ABPM$zero.rate
  )
